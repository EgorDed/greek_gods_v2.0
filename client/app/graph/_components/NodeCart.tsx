...
import type { IGraphEdge } from "@/app/graph/types";
import { getNodeRoleLabel } from "@/app/graph/_components/graphUtils";
import { NodeRelationsList } from "@/app/graph/_components/editor/NodeRelationsList";
import { NodeKinshipEditor } from "@/app/graph/_components/editor/NodeKinshipEditor";
...
const NodeCart = () => {
    ...
    const { graphData, selectedNode, setSelectedNode, isEditMode, authToken, refreshGraph } =
        useGraphContext();

    const edgesForNode = useMemo(() => {
        if (!selectedNode) {
            return [] as IGraphEdge[];
        }

        return graphData.edges.filter(
            (edge) => edge.from_node_id === selectedNode.id || edge.to_node_id === selectedNode.id,
        );
            ...
            <div className="px-4 space-y-4 pb-6">
                ...
                {isEditable && (
                    <NodeKinshipEditor
                        ...
                        allNodes={graphData.nodes as any}
                        token={authToken}
                        nodeTypes={NODE_TYPES}
                        genders={GENDERS}
